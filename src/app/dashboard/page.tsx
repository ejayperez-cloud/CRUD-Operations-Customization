...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen p-8 flex flex-col bg-gradient-to-r from-red-700 via-red-800 to-zinc-900">
          ...
          <form onSubmit={handleSubmit} className="mb-6 text-black font-semibold text-lg">
            ...
            <div className="flex flex-col w-full gap-4 mb-6">
              <Input
                ...
                className="rounded-full bg-red-200/50 text-black placeholder-black text-center"
              />
              <Input
                value={positionName}
          ...
          <div className="rounded-3xl overflow-hidden border border-purple-600">
            <table className="w-full text-black">
              ...
              <tbody className="bg-gradient-to-b from-gray-400 via-gray-600 to-red-800 text-white">
                {positions.map((p) => (
                  <tr key={p.position_id} className="text-center">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-3">
                        <Button
                          ...
                          className="bg-black text-white rounded-full px-5 py-1.5 w-24"
                        >
                          Edit
                        </Button>
                        <Button
                          onClick={() => {
                            fetch(`${API_BASE}/positions/${p.position_id}`, {
                              method: "DELETE",
                              headers: { Authorization: `Bearer ${getToken()}` },
                            }).then(fetchPositions);
                          }}
                          className="bg-red-600 text-white rounded-full px-5 py-1.5 w-24"
                        >
                          Delete
                        </Button>
                      </div>
      ...
      {/* Display bearer token at bottom of page */}
      {token && (
        <footer className="text-white text-sm text-center mt-8">
          <div className="break-all px-4 py-2 bg-black bg-opacity-50 rounded-lg max-w-full overflow-x-auto">
            <strong>Bearer Token:</strong> <code>{token}</code>
          </div>
        </footer>
      )}
    </div>
